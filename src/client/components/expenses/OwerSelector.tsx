...
import type { OwerInput } from '../../../shared/schemas/expense.schema';
...
export function OwerSelector({
  ...
  const validateSplit = (currentOwers: OwerInput[]) => {
    ...
    if (splitMethod === 'FIXED') {
      const total = currentOwers.reduce((sum, o) => sum + (o.splitValue || 0), 0);
      if (total !== baseAmount) {
        newErrors.push(
  ...
  return (
    <div>
      ...
      {owers.length === 0 && (
        <p className="mt-2 text-sm text-red-600">At least one person is required</p>
      )}
    </div>
  );
}
